-- ============================================================
-- Support system (tickets + threaded messages).
-- Apply on the self-hosted VPS:
--   docker exec -i supabase-db psql -U postgres -d postgres < selfhost/support.sql
...
DROP POLICY IF EXISTS "Users add messages to own tickets" ON public.support_messages;
...
CREATE INDEX IF NOT EXISTS idx_support_tickets_user ON public.support_tickets(user_id, last_activity_at DESC);
...
DROP TRIGGER IF EXISTS trg_support_activity ON public.support_messages;
