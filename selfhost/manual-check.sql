-- ============================================================
-- Manual checker + sold-card removal.
--  * A sold card is deactivated immediately (disappears from the shop).
--  * Refundable cards are NOT auto-checked any more: the check fee is still
--    charged at purchase, the check row is stored as 'pending'.
--  * public.run_card_checks(order_ids) resolves the pending checks using the
--    admin ratio (site_settings.refund_live_rate) and refunds DEAD cards.
-- Idempotent: safe to run multiple times.
-- ============================================================

ALTER TABLE public.card_checks DROP CONSTRAINT IF EXISTS card_checks_status_check;
ALTER TABLE public.card_checks
  ADD CONSTRAINT card_checks_status_check CHECK (status IN ('live','dead','pending'));
